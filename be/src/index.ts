...
import { GoogleGenerativeAI } from "@google/generative-ai"; // Google Gemini API SDK for Node.js
...
app.post("/template", async (req, res) => {
  ...
  if (answer === "react") {
    res.json({
      prompts: [
        BASE_PROMPT, // Assuming BASE_PROMPT is a general instruction
        `Here is an artifact that contains all files of the project visible to you.\nConsider the contents of ALL files in the project.\n\n${reactBasePrompt}\n\nHere is a list of files that exist on the file system but are not being shown to you:\n\n  - .gitignore\n  - package-lock.json\n`
      ],
      uiPrompts: [reactBasePrompt] // Prompts specifically for UI generation
    });
    return;
  }
  ...
  if (answer === "node") {
    res.json({
      ...
      uiPrompts: [nodeBasePrompt] // Prompts specifically for Node.js generation
    });
    return;
  }
